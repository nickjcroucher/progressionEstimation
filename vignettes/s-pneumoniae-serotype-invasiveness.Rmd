...
``` {r Plot precision parameters for negative binomial models}

nb_model_phi_values_df <- data.frame()

for (i in c(2,4,6,8)) {
  phi_df <- data.frame(
    "Model" = fit_list[[i]]@model_name,
    "phi" = model_outputs[[i]]$phi[1],
    "phi_lower" = model_outputs[[i]]$phi_lower[1],
    "phi_upper" = model_outputs[[i]]$phi_upper[1]
  )
  nb_model_phi_values_df %<>% dplyr::bind_rows(phi_df)
}

ggplot(nb_model_phi_values_df,
         aes(x = Model,
             y = phi,
             ymin = phi_lower,
             ymax = phi_upper)) +
...
get_serotype_stats <- function(serotype, df) {
  sero_df <-
    df %>%
      ...
      dplyr::mutate(matching_type_disease = dplyr::count(matching_type)))
