...
vignette: >
  %\VignetteIndexEntry{s-pneumoniae-serotype-invasiveness-adults}
...
ggsave(combined_rhat_plots,
       file = "adult_combined_rhat_plots.png",
       width = 8,
       height = 6)
...
ggsave(combined_trace_plots,
       file = "adult_combined_trace_plots.png",
       width = 8,
       height = 6)
...
ggsave(pred_v_obs_plot,
       file = "adult_serotypes_pred_v_obs_plot.png",
       height = 20,
       width = 10)
...
obs_pred_df <-
  ...
  dplyr::select(model_name,carriage_abs_dev,disease_abs_dev,phi,phi_lower,phi_upper) %>%
  tidyr::pivot_longer(c(carriage_abs_dev,disease_abs_dev), names_to = "Observation", values_to = "Abs_dev") %>%
  ...
  dplyr::mutate(Observation = dplyr::case_when(
                    Observation == "carriage_abs_dev" ~ "Carriage",
                    Observation == "disease_abs_dev" ~ "Disease"
                  )
                )

ggplot(obs_pred_df, aes(x = model_name, y = Abs_dev)) +
...
ggsave(phi_plot,
       file="adult_serotype_phi_plot.pdf",
       height = 6,
       width = 6)
...
``` {r Comparison with BF, include = TRUE}
...
(best_fitting_model_validation_plot <-
...
ggsave(best_fitting_model_validation_plot,
       file = "best_fitting_adult_model_validation_plot.png",
       width = 8,
       height = 6)
...
## Compare invasiveness values with those from children

``` {r Comparison with infant values}
...
joint_invasiveness <-
  infant_data %>%
    dplyr::rename(infant_nu = invasiveness) %>%
    dplyr::rename(infant_nu_lower = invasiveness_lower) %>%
    dplyr::rename(infant_nu_upper = invasiveness_upper) %>%
    dplyr::left_join(
      best_fitting_output %>%
          dplyr::select(type,nu,nu_lower,nu_upper) %>%
          dplyr::distinct(),
...
serotypes_to_label <- c("1","27","12F","12B","8","29","6C","18B","33A","10F","35A","11A")
...
joint_invasiveness_plot <-
  ggplot(joint_invasiveness,
    ...
    ggrepel::geom_label_repel(data = joint_invasiveness %>%
                                      dplyr::filter(type %in% serotypes_to_label),
                              aes(label = type)) +
    theme_bw() +
    theme(legend.position = "none")

overall_inv_analysis_plot <-
  cowplot::plot_grid(plotlist = list(invasiveness_plot,joint_invasiveness_plot),
                     nrow = 2,
                     ncol = 1,
                     labels = "AUTO")

ggsave(overall_inv_analysis_plot,
       file = "joint_child_adult_invasiveness_plot.pdf",
       height = 11,
       width = 8)
...
ggsave(scale_factor_plot,
       file = "adult_serotype_scale_factor_plot.png",
       width = 8,
       height = 6)
...
pv_full_adjusted_type_specific_negbin_fit <- 
...
trace_plot <- rstan::traceplot(pv_full_adjusted_type_specific_negbin_fit, pars = "lp__")
...
pv_best_fitting_output <- 
  progressionEstimation::process_progression_rate_model_output(
    pv_full_adjusted_type_specific_negbin_fit,                                                 pv_S_pneumoniae_adult_serotype) %>%
      dplyr::mutate("classification" = dplyr::case_when(
