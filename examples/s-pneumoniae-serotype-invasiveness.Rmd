...
pv_S_pneumoniae_infant_serotype <-
  S_pneumoniae_infant_serotype %>%
    dplyr::mutate(categorisation = dplyr::case_when(
                      categorisation %in% vaccine_types[["PCV7"]] & grepl("post.PCV", study) ~ paste0(categorisation,"_pv"),
                      categorisation == "6A" & grepl("post.PCV", study) ~ paste0(categorisation,"_pv"),
                      categorisation %in% vaccine_types[["PCV10"]] & grepl("post.PCV10", study) ~ paste0(categorisation,"_pv"),
                      categorisation %in% vaccine_types[["PCV10"]] & grepl("post.PCV13", study) ~ paste0(categorisation,"_pv"),
                      categorisation %in% vaccine_types[["PCV13"]] & grepl("post.PCV13", study) ~ paste0(categorisation,"_pv"),
                      categorisation == "6C" & grepl("post.PCV13", study) ~ paste0(categorisation,"_pv"),
                      TRUE ~ as.character(categorisation)
                    )
                  ) %>%
  dplyr::mutate(categorisation = factor(categorisation))
...
pv_best_fitting_output <- 
  progressionEstimation::process_progression_rate_model_output(
    pv_full_adjusted_type_specific_poisson_fit,                                                 pv_S_pneumoniae_infant_serotype) %>%
      dplyr::mutate("classification" = dplyr::case_when(
          gsub("_pv","",categorisation) %in% vaccine_types[["PCV7"]] ~ "PCV7",
          gsub("_pv","",categorisation) %in% vaccine_types[["PCV10"]] ~ "PCV10",
          gsub("_pv","",categorisation) %in% vaccine_types[["PCV13"]] ~ "PCV13",
          gsub("_pv","",categorisation) %in% vaccine_types[["PCV15"]] ~ "PCV15",
          gsub("_pv","",categorisation) %in% vaccine_types[["PCV20"]] ~ "PCV20",
          gsub("_pv","",categorisation) %in% vaccine_types[["PPV23"]] ~ "PPV23",
          TRUE ~ "No PCV"
        )
      ) %>%
      dplyr::mutate(classification = factor(classification, levels = c("PCV7",
...
pv_best_fitting_output %<>%
  dplyr::mutate(categorisation = factor(categorisation, levels = pv_serotype_levels))
...
sample_sizes <-
  best_fitting_output %>%
      ...
      dplyr::ungroup() %>%
      dplyr::select(categorisation, Sample_count) %>%
      dplyr::distinct() %>%
...
combined_invasiveness_df <-
  ...
  dplyr::left_join(sample_sizes, by = c("Serotype" = "categorisation"))
...
carriage_duration.df <- data.frame(
  "categorisation" = c("19F","23F","6B","14","21","19B","18C","29","3","4","24F","NT","5","6A","6C"),
...
carriage_duration.df %<>%
  dplyr::inner_join(pv_best_fitting_output,
                    by = "categorisation") %>%
  dplyr::select(categorisation,carriage_duration,classification,nu,nu_lower,nu_upper) %>%
  dplyr::distinct()
...
ggplot(carriage_duration.df,
  ...
  ggrepel::geom_label_repel(aes(label = categorisation)) +
  scale_y_continuous(trans = "log10") +
  theme_bw()
