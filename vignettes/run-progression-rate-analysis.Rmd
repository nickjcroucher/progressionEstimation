...
s_pneumoniae_netherlands_df <- progressionEstimation::process_input_xlsx("s_pneumoniae_netherlands.xlsx",
                                                                         use_strain = FALSE)
...
s_pneumoniae_netherlands_poisson_fit <- 
  progressionEstimation::fit_progression_rate_model(s_pneumoniae_netherlands_data,
                                                    type_specific = TRUE,
...
s_pneumoniae_netherlands_poisson_fit@model_pars
...
``` {r Check for MCMC convergence}

rstan::traceplot(s_pneumoniae_netherlands_poisson_fit,
                 pars = "nu_j")

```

Both MCMCs have converged on similar values, which suggests we have identified genuine variation between serotypes. This can be formally tested using `bayesplot`, which estimates $\hat{R}$ values across MCMCs:
...
s_pneumoniae_netherlands_poisson_fit %>%
    bayesplot::rhat() %>%
    bayesplot::mcmc_rhat() +
    bayesplot::yaxis_text()

```

All the parameters are estimated with $\hat{R}$ below the generally accepted threshold of 1.05 (in fact, they all line up along the 1.00 line), which confirms convergence of the parameter estimates. We can now combine the results of the model fit with the original data to enable the MCMCs outputs to be interpreted.

``` {r Process Poisson model output}

s_pneumoniae_netherlands_poisson_model_output_df <-
  progressionEstimation::process_progression_rate_model_output(s_pneumoniae_netherlands_df,
                                                               s_pneumoniae_netherlands_poisson_fit)
...
progressionEstimation::plot_case_carrier_predictions(s_pneumoniae_netherlands_poisson_model_output_df,
                                                     n_label = 3)
...
progressionEstimation::plot_progression_rates(s_pneumoniae_netherlands_poisson_model_output_df,
                                              unit_time = "year",
                                              type_name = "Serotype")
...
s_pneumoniae_netherlands_null_fit <- 
  progressionEstimation::fit_progression_rate_model(s_pneumoniae_netherlands_data,
                                                    type_specific = FALSE,
                                                    location_adjustment = FALSE,
                                                    num_chains = 2,
...
s_pneumoniae_netherlands_null_model_output_df <-
  progressionEstimation::process_progression_rate_model_output(s_pneumoniae_netherlands_df,
                                                               s_pneumoniae_netherlands_null_fit)
...
progressionEstimation::plot_case_carrier_predictions(s_pneumoniae_netherlands_null_model_output_df,
                                                     n_label = 3)
...
progressionEstimation::compare_model_fits_with_loo(list(s_pneumoniae_netherlands_null_fit,
                                 s_pneumoniae_netherlands_poisson_fit)) %>%
  kableExtra::kable()  %>%
...
progressionEstimation::compare_model_fits_with_bf(list(s_pneumoniae_netherlands_null_fit,
                                 s_pneumoniae_netherlands_poisson_fit)) %>%
